...
const Search = ({ query }: SearchProps) => {
    const { isLoading, error, results } = useSelector((state: any) => state.search);
    ...
    return (
        <div>
            ...
            {!isLoading && !query &&
                <div>
                    Not loading, no query
                </div>}

            {!isLoading && error &&
                <div>
                    {`Error ${error} occurred - see console for additional information`}
                </div>}
...
type SearchProps = InferPropTypes<
    typeof SearchPropTypes,
    typeof SearchDefaultProps
>;
